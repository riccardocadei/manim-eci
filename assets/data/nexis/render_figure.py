...
Phases:
  1. grow_base_XX.png   — t-test + Bonferroni bars grow (NEXIS hidden)
  2. trend.png          — same + red trend line on precision showing the paradox
  3. add_nexis_XX.png    — baselines dimmed, NEXIS (ours) grows in
...
# 3 methods, in bar order (baselines first, NEXIS last/right)
METHODS = [
    ("Marginal",       "#E07A5F", r"$t$-test"),
    ("Marginal (Bon)", "#81B29A", "Bonferroni"),
    ("NEXIS (auto)",    "#5BC4F5", "NEXIS (ours)"),
]
NEXIS_METHOD   = METHODS[2]
BASELINES     = METHODS[:2]
BASELINE_KEYS = {m[0] for m in BASELINES}
NEXIS_KEYS     = {NEXIS_METHOD[0]}
...
if __name__ == "__main__":
    ...
    # Phase 1: baselines grow (NEXIS hidden)
    ...
    for i in range(N_FRAMES):
        ...
        render_frame(
            ...
            height_scale=hs, hide_keys=NEXIS_KEYS,
            legend_methods=BASELINES,
        )

    # Phase 2: trend line on precision (static, baselines at full, NEXIS hidden)
    print("Phase 2: trend line ...")
    render_frame(
        df_e, df_n,
        os.path.join(FRAMES_DIR, "trend.png"),
        height_scale={m[0]: 1.0 for m in BASELINES},
        hide_keys=NEXIS_KEYS, draw_trend=True,
        legend_methods=BASELINES,
    )

    # Phase 3: baselines dim, NEXIS (ours) grows in
    print("Phase 3: NEXIS grows ...")
    for i in range(N_FRAMES):
        t = ease_out(i / (N_FRAMES - 1))
        ...
        render_frame(
            df_e, df_n,
            os.path.join(FRAMES_DIR, f"add_nexis_{i:02d}.png"),
