...
class Teaser(Scene):
    def construct(self):
        ...
        session.next_to(_dummy, DOWN, buff=0.55)

        tldr_prefix = Text("TL;DR:", color=GRAY_TEXT).scale(BODY_SCALE)
        tldr_content = Text(
            "enabling modern representation learning\n"
            "as a measurement model for scientific hypothesis generation",
            color=GRAY_TEXT,
            slant=ITALIC,
            line_spacing=0.6,
        ).scale(BODY_SCALE)
        tldr = Group(tldr_prefix, tldr_content).arrange(RIGHT, buff=0.2, aligned_edge=UP)
        tldr.next_to(session, DOWN, buff=0.45)

        logo_iclr = _whiten(ImageMobject(os.path.join(LOGO_DIR, "ICLR.png")))
        logo_iclr.scale_to_fit_height(ICLR_H)
        logo_iclr.move_to(np.array([4.0, 2.45, 0]))
        ...
        self.play(FadeIn(logo_iclr), run_time=0.5)
        self.play(FadeIn(session, shift=UP * 0.1), run_time=0.6)
        self.play(FadeIn(tldr, shift=UP * 0.1), run_time=0.8)
        self.wait(1.2)
