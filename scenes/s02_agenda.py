...
class S02Agenda(Slide):
    def construct(self):
        ...
        item1_sub = MarkupText(
            f'Exploratory Causal Inference in SAEnce — Mencattini{_STAR}, Cadei{_STAR}, Locatello '
            '\nICLR’26 — Oral (top ~1%)',
            color=GRAY_TEXT, slant=ITALIC,
        ).scale(0.32)
        ...
        # ── Items appear ────────────────────────────────────────────────────
        self.play(FadeIn(item1, shift=UP * 0.12), run_time=0.6)
        self.play(FadeIn(item2, shift=UP * 0.12), run_time=0.6)
        self.wait(0.5)
        self.next_slide()
        ...
        self.play(
            item2.animate.set_opacity(0.25),
            Transform(item1_main, item1_main_target),
            run_time=0.6,
        )
        self.wait(0.5)
        self.next_slide()
